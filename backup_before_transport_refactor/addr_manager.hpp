...
struct AddrInfo {
    ...
    AddrInfo() : timestamp(0), last_try(0), last_success(0), attempts(0), tried(false) {}
    explicit AddrInfo(const protocol::NetworkAddress& addr, uint32_t ts = 0)
        : address(addr), timestamp(ts), last_try(0), last_success(0), attempts(0), tried(false) {}
 ...
 * 1. Bucket-based architecture (256 tried × 64, 1024 new × 64) instead of simple maps
 * 2. Cryptographic bucket selection using secret key to prevent targeted attacks
 * 3. NetGroupManager for network group diversity (/16 IPv4, /32 IPv6, ASN-based)
 ...
 * address table with their nodes). The bucket system provides anti-sybil protection
 * by limiting how many addresses from the same network can be stored.
 */
...
public:
    ...
    std::vector<protocol::TimestampedAddress> get_addresses(size_t max_count = 1000);
