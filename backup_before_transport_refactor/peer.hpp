...
private:
    ...
    void start_read_payload(const protocol::MessageHeader& header);
    ...
    // Send queue (protected by mutex since accessed from multiple threads)
    std::mutex send_queue_mutex_;
    std::queue<std::vector<uint8_t>> send_queue_;
    bool writing_ = false;

    // Receive buffers
    std::vector<uint8_t> recv_header_buffer_;
    std::vector<uint8_t> recv_payload_buffer_;
