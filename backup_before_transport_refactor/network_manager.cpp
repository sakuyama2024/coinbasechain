    ...
    if (config_.listen_enabled && config_.listen_port > 0) {
        try {
            acceptor_ = std::make_unique<boost::asio::ip::tcp::acceptor>(
                io_context_,
                boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), config_.listen_port)
            );
            start_accepting();
            LOG_NET_INFO("Listening on port {}", config_.listen_port);
        } catch (const std::exception& e) {
            LOG_NET_ERROR("Failed to start listener: {}", e.what());
...
bool NetworkManager::connect_to(const std::string& address, uint16_t port) {
    ...
    // Create outbound peer with our local nonce
    auto peer = Peer::create_outbound(io_context_, address, port, config_.network_magic, local_nonce_);
    if (!peer) {
        return false;
    }
...
void NetworkManager::start_accepting() {
    if (!acceptor_ || !running_.load(std::memory_order_acquire)) {
        return;
    }
...
void NetworkManager::handle_accept(const boost::system::error_code& ec,
                                   boost::asio::ip::tcp::socket socket) {
    if (ec) {
        if (running_.load(std::memory_order_acquire)) {
    ...
    try {
        ...
        LOG_NET_WARN("Failed to get remote address: {}", e.what());
        socket.close();
        start_accepting();
        return;
    }
    ...
    auto peer = Peer::create_inbound(io_context_, std::move(socket), config_.network_magic, local_nonce_);
    if (peer) {
        // Setup message handler
        setup_peer_message_handler(peer.get());
...
bool NetworkManager::handle_getheaders_message(PeerPtr peer, message::GetHeadersMessage* msg) {
    ...
    while (pindex && response->headers.size() < protocol::MAX_HEADERS_SIZE) {
        response->headers.push_back(pindex->GetBlockHeader());
