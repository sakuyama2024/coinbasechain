...
namespace chain { class CBlockIndex; }
namespace sync { class HeaderSync; }

/**
 * Modern C++20 notification system for blockchain events
 *
 * Design philosophy:
 ...
 * - No wallet events (we don't have wallet)
 * - No async queue (simpler, fewer threads)
...
public:
    ...
    // Callback types
    using BlockConnectedCallback = std::function<void(const CBlockHeader& block, const chain::CBlockIndex* pindex)>;
    using BlockDisconnectedCallback = std::function<void(const CBlockHeader& block, const chain::CBlockIndex* pindex)>;
    using ChainTipCallback = std::function<void(const chain::CBlockIndex* pindexNew, int height)>;
     ...
     * Returns RAII subscription handle
     */
    [[nodiscard]] Subscription SubscribeBlockDisconnected(BlockDisconnectedCallback callback);
     ...
     *
     * CRITICAL NOTIFICATION SEMANTICS (matching Bitcoin Core):
     ...
     *
     * Example:
     *   void OnBlockConnected(const CBlockHeader& block, const CBlockIndex* pindex) {
     *       // GetTip() now returns pindex (the newly connected block)
     *       assert(chainstate.GetTip() == pindex);
     *   }
     */
    void NotifyBlockConnected(const CBlockHeader& block, const chain::CBlockIndex* pindex);
     ...
     *
     * IMPORTANT: This is ASYMMETRIC with NotifyBlockConnected() - subscribers must handle:
     *   - On BlockConnected: GetTip() == newly connected block
     *   - On BlockDisconnected: GetTip() == block being disconnected
     *
     * Example:
     *   void OnBlockDisconnected(const CBlockHeader& block, const CBlockIndex* pindex) {
     *       // GetTip() still returns pindex (the block being removed)
     *       assert(chainstate.GetTip() == pindex);
     *       // After this callback, tip will move to pindex->pprev
     *   }
     */
    void NotifyBlockDisconnected(const CBlockHeader& block, const chain::CBlockIndex* pindex);
