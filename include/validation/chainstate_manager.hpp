...
// Processes headers, activates best chain, emits notifications (simplified from Bitcoin Core)
...
public:
    ...
    // CRITICAL ANTI-DOS: Cheap commitment PoW check BEFORE index, full PoW AFTER (cached if fails)
    // ORPHAN HANDLING: Missing parent → cached as orphan (DoS limits), auto-processed when parent arrives
    // Returns nullptr if orphaned (state="orphaned") or failed (state="invalid")
...
private:
    // Connect/disconnect blocks (headers-only: just updates chain state and notifications)
    ...
    // Orphan header storage (headers with missing parent, auto-processed when parent arrives)
    struct OrphanHeader {
        CBlockHeader header;
    ...
    // Not protected: m_cached_finished_ibd (atomic), params_ (const), suspicious_reorg_depth_ (const)
