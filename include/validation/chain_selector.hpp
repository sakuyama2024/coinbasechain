...
// Order: 1) Higher chain work, 2) Higher height, 3) Lexicographic hash (deterministic tie-breaker)
//
...
// Maintains set of leaf nodes (validated to BLOCK_VALID_TREE) that could be chain tips
// Selects best chain by most accumulated work, prunes stale candidates
//
// THREAD SAFETY: No internal mutex - caller (ChainstateManager) must hold validation_mutex_
class ChainSelector {
public:
    ...
    // When block extends candidate, parent is auto-removed (maintains leaf-only invariant)
    // Caller must hold validation_mutex_
    void TryAddBlockIndexCandidate(chain::CBlockIndex* pindex,
                                    const chain::BlockManager& block_manager);

    // Prune stale candidates (less work than tip, active tip itself, ancestors, non-leaves)
    // Caller must hold validation_mutex_
    ...
    // Update best header if new block has more work (caller must hold validation_mutex_)
