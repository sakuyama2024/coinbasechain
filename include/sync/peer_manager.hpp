...
static constexpr int DISCOURAGEMENT_THRESHOLD =
    100; // Misbehavior score threshold for disconnection
...
static constexpr int TOO_MANY_UNCONNECTING =
    20; // Too many unconnecting headers messages
static constexpr int TOO_MANY_ORPHANS =
    50; // Exceeded orphan header limit - moderate penalty
} // namespace MisbehaviorPenalty

// Maximum unconnecting headers messages before penalty
static constexpr int MAX_UNCONNECTING_HEADERS = 10;

// NetPermissionFlags - Permission flags for peer connections (from Bitcoin
// Core)
...
inline NetPermissionFlags operator|(NetPermissionFlags a,
                                    NetPermissionFlags b) {
  return static_cast<NetPermissionFlags>(static_cast<uint32_t>(a) |
...
struct Peer {
  ...
  int num_unconnecting_headers_msgs{
      0}; // Counter for headers that don't connect
