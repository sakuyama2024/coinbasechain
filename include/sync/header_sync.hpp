...
// Internal state protected by mutex_ (held only for brief reads/writes, not
// during validation) LOCKING ORDER: ChainstateManager::validation_mutex_ FIRST,
// HeaderSync::mutex_ AFTER
class HeaderSync {
public:
  enum class State {
    IDLE,    // Not syncing
  ...
  HeaderSync(validation::ChainstateManager &chainstate_manager,
  ...
  // Get block locator from pprev of tip (ensures non-empty response even if
  // peer at same tip)
  ...
  // Get sync progress (0.0 to 1.0, estimated from tip timestamp vs current
  // time)
  ...
  // Should we request more headers? (true if last batch was full and we're not
  // synced)
...
private:
  ...
  // See LOCKING_ORDER.md: Acquire ChainstateManager::validation_mutex_ BEFORE
  // this
