...
struct ConsensusParams {
    ...
    // Set to 1 to use block 1 as anchor (block 0=genesis and block 1 both use powLimit)
    // This allows block 1 to be mined at any time without difficulty adjustment issues
    int32_t nASERTAnchorHeight{1};

    // Hash of genesis block
    uint256 hashGenesisBlock;

    // Minimum cumulative chain work for IBD completion (eclipse attack protection)
...
CBlockHeader CreateGenesisBlock(
    uint32_t nTime,
    uint32_t nNonce,
    uint32_t nBits,
    int32_t nVersion = 1
