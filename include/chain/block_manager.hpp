...
// Simplified from Bitcoin Core for headers-only chain (no LevelDB, block files, UTXO, tx index, or pruning)
// All headers kept in memory (~120 bytes each: 1M headers = ~120 MB, 10M headers = ~1.2 GB)
//
// THREAD SAFETY: NO internal mutex - caller MUST hold ChainstateManager::validation_mutex_
// BlockManager is PRIVATE member of ChainstateManager, all access goes through ChainstateManager
...
public:
    ...
    // Add new block header to index (returns pointer to CBlockIndex, existing or new)
    // Creates CBlockIndex, sets parent pointer, calculates height and chain work
...
private:
    // Map of all known blocks: hash -> CBlockIndex (map owns CBlockIndex objects, keys are what phashBlock points to)
