...
// TransportConnection - Abstract interface for sending/receiving data over a connection
// Implementations handle actual I/O (TCP socket, in-memory, etc.)
class TransportConnection {
public:
    virtual ~TransportConnection() = default;

    // Start receiving data (callbacks invoked when data arrives or connection closes)
...
// Implementations provide both outbound connection initiation and inbound acceptance
class Transport {
public:
    virtual ~Transport() = default;

    // Initiate outbound connection (callback called on success/fail, returns connection object)
    ...
    // Start accepting inbound connections (returns true if listening started successfully)
    virtual bool listen(
        uint16_t port,
    ...
    // Run transport event loop (blocks until stop() called, or returns immediately for sync transports)
    virtual void run() = 0;
