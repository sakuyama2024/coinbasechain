...
TEST_CASE("BlockManager Defensive - Tip Selection", "[chain][block_manager][defensive]") {
    ...
    SECTION("Ignore invalid blocks when selecting tip") {
        json root = fixture.CreateValidChainJSON(3);

        // Find and mark the block at height 2 as invalid
        for (auto& block : root["blocks"]) {
            if (block["height"] == 2) {
                block["status"] = BLOCK_FAILED_VALID;
                break;
            }
        }
        ...
        // Should select valid chain (height 1) not invalid tip (height 2)
