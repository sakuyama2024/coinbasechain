...
TEST_CASE("InvalidateBlock (chain) - Basic invalidation", "[invalidateblock][chain]") {
    ...
    bool ok=fx.chainstate.InvalidateBlock(b2); REQUIRE(ok); auto* b2i=fx.Get(b2); REQUIRE(b2i); CHECK(b2i->nStatus & BLOCK_FAILED_VALID); auto* b3i=fx.Get(b3); REQUIRE(b3i); CHECK(b3i->nStatus & BLOCK_FAILED_CHILD); auto* b1i=fx.Get(b1); REQUIRE(b1i); CHECK(b1i->IsValid()); tip=fx.chainstate.GetTip(); REQUIRE(tip); CHECK(tip->nHeight==1); CHECK(tip->GetBlockHash()==b1);
}
