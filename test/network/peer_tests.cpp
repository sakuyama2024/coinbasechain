...
TEST_CASE("Peer - OutboundHandshake", "[peer][handshake]") {
    ...
    auto peer = Peer::create_outbound(io_context, mock_conn, magic, local_nonce);
...
TEST_CASE("Peer - InboundHandshake", "[peer][handshake]") {
    ...
    auto peer = Peer::create_inbound(io_context, mock_conn, magic, local_nonce);
...
TEST_CASE("Peer - SendMessage", "[peer][messages]") {
    ...
    const uint32_t magic = protocol::magic::REGTEST;
    auto peer = Peer::create_outbound(io_context, mock_conn, magic, 12345);
...
TEST_CASE("Peer - UserAgentLengthValidation", "[peer][security]") {
    ...
    const uint32_t magic = protocol::magic::REGTEST;
    auto peer = Peer::create_inbound(io_context, mock_conn, magic, 12345);
    peer->start();
    io_context.poll();
