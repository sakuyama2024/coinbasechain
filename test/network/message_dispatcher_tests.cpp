...
TEST_CASE("MessageDispatcher - Basic registration and dispatch", "[network][message_dispatcher]") {
  ...
  SECTION("Register and dispatch handler") {
    ...
    dispatcher.RegisterHandler("test", [&](PeerPtr peer, ::coinbasechain::message::Message* msg) {
      handler_called = true;
    ...
    coinbasechain::message::TestMessage msg;
...
TEST_CASE("MessageDispatcher - Multiple handlers", "[network][message_dispatcher]") {
  ...
  dispatcher.RegisterHandler("verack", [&](PeerPtr, ::coinbasechain::message::Message*) {
    ++verack_count;
    return true;
  });

  dispatcher.RegisterHandler("ping", [&](PeerPtr, ::coinbasechain::message::Message*) {
    ++ping_count;
    return true;
  });

  dispatcher.RegisterHandler("inv", [&](PeerPtr, ::coinbasechain::message::Message*) {
    ++inv_count;
    return true;
  });
...
TEST_CASE("MessageDispatcher - Handler return values", "[network][message_dispatcher]") {
  ...
  SECTION("Handler returning false propagates") {
    dispatcher.RegisterHandler("fail", [](PeerPtr, ::coinbasechain::message::Message*) {
      return false;
    });
  ...
  SECTION("Handler returning true propagates") {
    dispatcher.RegisterHandler("success", [](PeerPtr, ::coinbasechain::message::Message*) {
      return true;
    });
...
TEST_CASE("MessageDispatcher - Handler exceptions", "[network][message_dispatcher]") {
  ...
  SECTION("Exception in handler is caught and returns false") {
    dispatcher.RegisterHandler("throws", [](PeerPtr, ::coinbasechain::message::Message*) -> bool {
...
TEST_CASE("MessageDispatcher - Unregister", "[network][message_dispatcher]") {
  ...
  int call_count = 0;
  dispatcher.RegisterHandler("test", [&](PeerPtr, ::coinbasechain::message::Message*) {
    ++call_count;
    return true;
  });
    ...
    for (int i = 0; i < 10; ++i) {
      threads.emplace_back([&dispatcher, i]() {
        dispatcher.RegisterHandler("cmd" + std::to_string(i),
          [](PeerPtr, ::coinbasechain::message::Message*) { return true; });
      });
    }
