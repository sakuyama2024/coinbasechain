...
TEST_CASE("SlotExhaustion - PROOF: Attacker can fill inbound slots", "[network][vulnerability][slotexhaustion]") {
    ...
    const int ATT=10; std::vector<std::unique_ptr<AttackSimulatedNode>> attackers; attackers.reserve(ATT);
    for(int i=0;i<ATT;i++){ attackers.push_back(std::make_unique<AttackSimulatedNode>(100+i,&network)); attackers.back()->SetBypassPOWValidation(true); attackers.back()->ConnectTo(1);} 
...
TEST_CASE("SlotExhaustion - PROOF: Rotation attack maintains protection", "[network][vulnerability][slotexhaustion][rotation]") {
    ...
    const int N=5; std::vector<std::unique_ptr<AttackSimulatedNode>> attackers; for(int i=0;i<N;i++){ attackers.push_back(std::make_unique<AttackSimulatedNode>(100+i,&network)); attackers.back()->SetBypassPOWValidation(true); attackers.back()->ConnectTo(1);} 
TestOrchestrator orch(&network);
...
TEST_CASE("SlotExhaustion - PROOF: Honest peer blocked when slots full", "[network][vulnerability][slotexhaustion]") {
    ...
    SimulatedNode victim(1,&network); victim.SetBypassPOWValidation(true); for(int i=0;i<5;i++) victim.MineBlock();
    const int N=10; std::vector<std::unique_ptr<AttackSimulatedNode>> attackers; for(int i=0;i<N;i++){ attackers.push_back(std::make_unique<AttackSimulatedNode>(100+i,&network)); attackers.back()->SetBypassPOWValidation(true); attackers.back()->ConnectTo(1);} 
TestOrchestrator orch(&network);
    REQUIRE(orch.WaitForCondition([&]{ return victim.GetPeerCount()==(size_t)N; }, std::chrono::seconds(5)));
...
TEST_CASE("SlotExhaustion - PROOF: Minimal resources needed", "[network][vulnerability][slotexhaustion]") {
    ...
    SimulatedNode victim(1,&network); victim.SetBypassPOWValidation(true); for(int i=0;i<5;i++) victim.MineBlock();
    const int N=8; std::vector<std::unique_ptr<AttackSimulatedNode>> attackers; for(int i=0;i<N;i++){ attackers.push_back(std::make_unique<AttackSimulatedNode>(100+i,&network)); attackers.back()->SetBypassPOWValidation(true); REQUIRE(attackers.back()->ConnectTo(1)); }
TestOrchestrator orch(&network);
    REQUIRE(orch.WaitForCondition([&]{ return victim.GetPeerCount()==(size_t)N; }, std::chrono::seconds(5)));
}
