...
TEST_CASE("No per-IP limit allows 50+ connections from same IP", "[dos][network][per-ip]") {
    ...
    // Victim should show NUM_ATTACKERS inbound peers (vulnerability demonstration)
    auto &pm = victim.GetNetworkManager().peer_manager();
    auto inbound = pm.get_inbound_peers();
    REQUIRE(static_cast<int>(inbound.size()) >= 50);
    ...
    // Expected policy: enforce a small per-IP limit (e.g., 2). This should FAIL until implemented.
