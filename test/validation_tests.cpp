...
TEST_CASE("GetAntiDoSWorkThreshold - DoS protection", "[validation]") {
    ...
    SECTION("Returns 0 during IBD") {
        bool is_ibd = true;
        arith_uint256 threshold = GetAntiDoSWorkThreshold(nullptr, *params, is_ibd);
        REQUIRE(threshold == 0);
    }
    ...
    SECTION("Returns value with valid tip") {
        ...
        bool is_ibd = false;
        arith_uint256 threshold = GetAntiDoSWorkThreshold(&tip, *params, is_ibd);
...
TEST_CASE("Validation constants", "[validation]") {
    ...
    SECTION("MAX_HEADERS_RESULTS is reasonable") {
