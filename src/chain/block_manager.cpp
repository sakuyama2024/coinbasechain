...
bool BlockManager::Initialize(const CBlockHeader& genesis)
{
    ...
    LOG_CHAIN_INFO("BlockManager initialized with genesis: {}", m_genesis_hash.ToString());
...
bool BlockManager::Load(const std::string& filepath, const uint256& expected_genesis_hash)
{
    ...
    try {
        ...
        LOG_CHAIN_INFO("Loading {} headers, genesis: {}, tip: {}",
                       block_count, genesis_hash_str, tip_hash_str);
        ...
        if (loaded_genesis_hash != expected_genesis_hash) {
            LOG_CHAIN_ERROR("GENESIS MISMATCH: Loaded genesis {} does not match expected genesis {}",
        ...
        std::map<uint256, std::pair<CBlockIndex*, uint256>> block_map;  // hash -> (pindex, prev_hash)
        ...
        for (auto& [hash, data] : block_map) {
            ...
            if (!prev_hash.IsNull()) {
                ...
                if (!pindex->pprev) {
                    LOG_CHAIN_ERROR("Parent block not found for {}: {}",
                                   hash.ToString(), prev_hash.ToString());
                    return false;
                }
            } else {
