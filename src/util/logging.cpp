...
void LogManager::Initialize(const std::string& log_level,
                           bool log_to_file,
                           const std::string& log_file_path)
{
    ...
    try {
        ...
        // NOTE: Console sink disabled to avoid stdout blocking when run from subprocess
        if (log_to_file) {
        ...
        std::vector<std::string> components = {
            "default", "network", "sync", "chain", "crypto", "app"
        };

        for (const auto& component : components) {
            auto logger = std::make_shared<spdlog::logger>(component, sinks.begin(), sinks.end());
            logger->set_level(spdlog::level::from_str(log_level));
            logger->flush_on(spdlog::level::debug);  // Flush all logs immediately (including DEBUG)
        ...
        // Add visual separator and banner for new session
        if (log_to_file) {
            for (int i = 0; i < 10; ++i) {
        ...
        spdlog::default_logger()->info("║    ╚═════╝ ╚═╝  ╚═══╝╚═╝ ╚═════╝╚═╝   ╚═╝      ╚═╝     ║");
        spdlog::default_logger()->info("║                                                          ║");
        spdlog::default_logger()->info("║                    P O W   C H A I N                     ║");
        spdlog::default_logger()->info("║                                                          ║");
        spdlog::default_logger()->info("╚══════════════════════════════════════════════════════════╝");
        spdlog::default_logger()->info("");
