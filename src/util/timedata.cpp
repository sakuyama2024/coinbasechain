...
void AddTimeData(const std::string& peer_addr, int64_t nOffsetSample)
{
    ...
    LOG_DEBUG("Added time data from peer {}: offset={:+d}s ({:+d} minutes), total samples={}",
             peer_addr,
             nOffsetSample,
             nOffsetSample / 60,
             g_time_offsets.size());
    ...
    if (g_time_offsets.size() >= 5 && g_time_offsets.size() % 2 == 1) {
        ...
        // This protects against eclipse attacks where attacker controls all our peers
        int64_t max_adjustment = DEFAULT_MAX_TIME_ADJUSTMENT;

        if (nMedian >= -max_adjustment && nMedian <= max_adjustment) {
            nTimeOffset = nMedian;
            LOG_INFO("Network time offset updated: {:+d}s ({:+d} minutes) based on {} samples",
                    nTimeOffset,
                    nTimeOffset / 60,
                    g_time_offsets.size());
        } else {
            ...
            if (!g_warning_emitted) {
                ...
                if (!fMatch) {
                    ...
                    LOG_ERROR("WARNING: Please check that your computer's date and time are correct! "
                             "If your clock is wrong, Coinbase Chain will not work properly.");
                    LOG_ERROR("Your clock differs from network time by more than {:+d} minutes (max adjustment). "
        ...
        if (g_time_offsets.size() >= 5) {
            ...
            log_message += "| median offset = " + std::to_string(nTimeOffset) + "s (" +
                          std::to_string(nTimeOffset / 60) + " minutes)";
