...
uint256 CBlockHeader::GetHash() const noexcept
{
    ...
    // in little-endian format. Reverse big-endian digest into little-endian storage.
    uint256 out;
...
CBlockHeader::HeaderBytes CBlockHeader::SerializeFixed() const noexcept
{
    ...
    // uint256 stores bytes in internal format, copy directly to wire (no endian swap)
    std::copy(hashPrevBlock.begin(), hashPrevBlock.end(), data.begin() + OFF_PREV);

    // minerAddress (20 bytes, offset 36)
    // uint160 stores bytes in internal format, copy directly to wire (no endian swap)
