...
void NetworkManager::run_maintenance() {
  ...
  if (current_sync_peer != 0) {
    ...
    int64_t now_us = std::chrono::duration_cast<std::chrono::microseconds>(
                         ...
                         .count();
    int64_t last_headers_us =
        last_headers_received_.load(std::memory_order_acquire);

    int64_t time_since_last_headers_sec = (now_us - last_headers_us) / 1000000;

    // Timeout: 60 seconds without receiving headers
    // (Bitcoin uses dynamic timeout based on chain age, we use simple fixed
    // timeout)
    constexpr int64_t SYNC_TIMEOUT_SECONDS = 60;

    if (time_since_last_headers_sec > SYNC_TIMEOUT_SECONDS) {
      LOG_NET_WARN("Header sync timeout! No headers received for {} seconds "
                   "from peer {}",
...
bool NetworkManager::handle_message(PeerPtr peer,
  ...
  if (command == protocol::commands::VERACK) {
    ...
    if (peer->successfully_connected()) {
      ...
      if (sync_peer_id_.compare_exchange_strong(expected, peer->id(),
        ...
        // Send GETHEADERS to initiate sync (like Bitcoin's "initial
        // getheaders")
        request_headers_from_peer(peer);
      }
    }
  ...
  if (command == protocol::commands::GETHEADERS) {
    auto *getheaders_msg =
        dynamic_cast<message::GetHeadersMessage *>(msg.get());
    if (getheaders_msg) {
