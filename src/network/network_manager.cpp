...
NetworkManager::NetworkManager(
    ...
    : config_(config),
      ...
      feeler_rng_(std::random_device{}()),  // Initialize feeler RNG once at construction
...
void NetworkManager::stop() {
  ...
  if (!config_.datadir.empty()) {
    try {
      ...
      LOG_NET_ERROR("Failed to save anchors during shutdown: {}", e.what());
    } catch (...) {
      LOG_NET_ERROR("Unknown exception while saving anchors during shutdown");
    }
  }
...
ConnectionResult NetworkManager::connect_to_with_permissions(const protocol::NetworkAddress &addr, NetPermissionFlags permissions) {
  ...
  return peer_manager_->ConnectTo(
      ...
      permissions,
      transport_,
      [this](const protocol::NetworkAddress& a) { discovery_manager_->Good(a); },
...
void NetworkManager::setup_peer_message_handler(Peer *peer) {
  ...
  // No need to register peer - network::ConnectionManager already tracks all peers
  // Misbehavior tracking is now handled by the unified ConnectionManager
}
