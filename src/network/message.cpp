...
void MessageSerializer::write_uint8(uint8_t value) {
...
void MessageSerializer::write_bool(bool value) {
...
void MessageSerializer::write_network_address(const protocol::NetworkAddress& addr, bool include_timestamp) {
    if (include_timestamp) {
        // Timestamp would be written by caller
    }
    write_uint64(addr.services);
    write_bytes(addr.ip.data(), addr.ip.size());
    endian::WriteBE16(buffer_.data() + buffer_.size() - 2, 0);  // Make space for port
...
bool MessageDeserializer::read_bool() {
    return read_uint8() != 0;
}
...
protocol::NetworkAddress MessageDeserializer::read_network_address(bool has_timestamp) {
    if (has_timestamp) {
...
std::vector<uint8_t> serialize_header(const protocol::MessageHeader& header) {
    ...
    std::memcpy(buffer.data() + pos, header.command.data(), protocol::COMMAND_SIZE);
    pos += protocol::COMMAND_SIZE;

    endian::WriteLE32(buffer.data() + pos, header.length);
    pos += 4;

    std::memcpy(buffer.data() + pos, header.checksum.data(), protocol::CHECKSUM_SIZE);
...
std::unique_ptr<Message> create_message(const std::string& command) {
    if (command == protocol::commands::VERSION) return std::make_unique<VersionMessage>();
    if (command == protocol::commands::VERACK) return std::make_unique<VerackMessage>();
    if (command == protocol::commands::PING) return std::make_unique<PingMessage>();
    if (command == protocol::commands::PONG) return std::make_unique<PongMessage>();
    if (command == protocol::commands::ADDR) return std::make_unique<AddrMessage>();
    if (command == protocol::commands::GETADDR) return std::make_unique<GetAddrMessage>();
    if (command == protocol::commands::INV) return std::make_unique<InvMessage>();
    if (command == protocol::commands::GETDATA) return std::make_unique<GetDataMessage>();
    if (command == protocol::commands::NOTFOUND) return std::make_unique<NotFoundMessage>();
    if (command == protocol::commands::GETHEADERS) return std::make_unique<GetHeadersMessage>();
    if (command == protocol::commands::HEADERS) return std::make_unique<HeadersMessage>();
...
bool GetHeadersMessage::deserialize(const uint8_t* data, size_t size) {
    ...
    if (stop_hash.size() != 32) return false;
