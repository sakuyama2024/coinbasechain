...
MessageHeader::MessageHeader(uint32_t magic, const std::string& cmd, uint32_t len)
...
NetworkAddress::NetworkAddress(uint64_t svcs, const std::array<uint8_t, 16>& addr, uint16_t p)
    : services(svcs), ip(addr), port(p) {
}

NetworkAddress NetworkAddress::from_ipv4(uint64_t services, uint32_t ipv4, uint16_t port) {
    NetworkAddress addr;
...
TimestampedAddress::TimestampedAddress()
    : timestamp(0) {
}
...
InventoryVector::InventoryVector(InventoryType t, const std::array<uint8_t, 32>& h)
    : type(t), hash(h) {
}
