...
HeaderSync::HeaderSync(validation::ChainstateManager &chainstate_manager,
                       const chain::ChainParams &params)
    : chainstate_manager_(chainstate_manager), params_(params),
      state_(State::IDLE), last_batch_size_(0),
      peer_manager_(std::make_unique<PeerManager>()) {
  LOG_INFO("HeaderSync initialized (sequential mode) with peer misbehavior "
           "tracking");
}
...
bool HeaderSync::ProcessHeaders(const std::vector<CBlockHeader> &headers,
                                int peer_id) {
  if (headers.empty()) {
    ...
    // if we should stop syncing (empty after full batch = synced) or keep
    // trying
    UpdateState();
    return true;
  }
  ...
  if (headers.size() > validation::MAX_HEADERS_RESULTS) {
    LOG_ERROR("HeaderSync: Rejecting oversized headers message from peer {} "
              "(size={}, max={})",
  ...
  LOG_INFO("HeaderSync: Processing {} headers from peer {}", headers.size(),
           peer_id);

  // DoS Protection: Check if first header connects to known chain
  const uint256 &first_prev = headers[0].hashPrevBlock;
  bool prev_exists =
      chainstate_manager_.LookupBlockIndex(first_prev) != nullptr;

  if (!prev_exists) {
    LOG_WARN("HeaderSync: Headers don't connect to known chain from peer {} "
             "(first header prevhash: {})",
    ...
    if (peer_manager_->IncrementUnconnectingHeaders(peer_id)) {
      ...
      peer_manager_->Misbehaving(peer_id,
                                 MisbehaviorPenalty::TOO_MANY_UNCONNECTING,
  ...
  // DoS Protection: Cheap PoW commitment check (before expensive full
  // validation) This is CRITICAL for DoS protection:
  ...
  if (!pow_ok) {
    LOG_ERROR("HeaderSync: Headers failed PoW commitment check from peer {}",
              peer_id);
  ...
  // - Reject headers with insufficient total work UNLESS they're ancestors of
  // best header/tip
  // - This blocks low-work spam forks while allowing legitimate chain
  // extensions
  //
  // Our Simplified Approach:
  // - We SKIP the Anti-DoS check entirely because headers already passed
  // connection check above
  // - If first_prev exists in our index, headers build on our known chain →
  // legitimate
  // - If first_prev doesn't exist, headers already rejected above (line 67-78)
  //
  // Why This Is Safe:
  // 1. Headers connect to our chain (first_prev verified above) → not random
  // spam
  // 2. Headers passed PoW commitment check → not trivially mined
  // 3. Headers are continuous → not corrupted
  // 4. Limited to MAX_HEADERS_RESULTS (2000) per message → bounded resource
  // usage
  //
  // Security Trade-off:
  // - Allows low-work forks from any point in our chain (attackers can fork
  // from genesis)
  ...
  //
  // Note: During IBD, GetAntiDoSWorkThreshold returns 0 anyway, so threshold
  // would be 0
  ...
  // This ensures we calculate reorg depth against the starting point, not
  // incrementally
  ...
  // (Similar to Bitcoin's approach where headers are validated but activation
  // is deferred)
  ...
  for (const auto &header : headers) {
    ...
    if (!pindex) {
      ...
      // Bitcoin: BLOCK_MISSING_PREV - not punished, we don't have the parent
      // yet
      if (reason == "orphaned") {
      ...
      // Bitcoin: Would be tracked separately, but we treat excessive orphans as
      // DoS
      if (reason == "orphan-limit") {
        LOG_WARN("HeaderSync: Peer {} exceeded orphan limit", peer_id);
        peer_manager_->Misbehaving(peer_id,
                                   MisbehaviorPenalty::TOO_MANY_ORPHANS,
                                   "exceeded orphan header limit");
        return false;
      }

      // Invalid header - instant disconnect (Bitcoin: BLOCK_INVALID_HEADER =
      // 100 points) Covers: "high-hash" (PoW failed), "bad-diffbits",
      // "time-too-old", "time-too-new", "bad-version"
      if (reason == "high-hash" || reason == "bad-diffbits" ||
          reason == "time-too-old" || reason == "time-too-new" ||
          reason == "bad-version") {
        LOG_ERROR("HeaderSync: Peer {} sent invalid header: {}", peer_id,
                  reason);
      ...
      // Duplicate/cached invalid - instant disconnect (Bitcoin:
      // BLOCK_CACHED_INVALID = 100 points)
      if (reason == "duplicate") {
        LOG_WARN("HeaderSync: Peer {} sent duplicate header marked invalid",
                 peer_id);
        peer_manager_->Misbehaving(peer_id, MisbehaviorPenalty::INVALID_HEADER,
      ...
      // Invalid parent - instant disconnect (Bitcoin: BLOCK_INVALID_PREV = 100
      // points)
      if (reason == "bad-prevblk") {
        LOG_ERROR("HeaderSync: Peer {} sent header with invalid parent",
                  peer_id);
        peer_manager_->Misbehaving(peer_id, MisbehaviorPenalty::INVALID_HEADER,
      ...
      if (reason == "bad-genesis" || reason == "genesis-via-accept") {
        LOG_ERROR("HeaderSync: Peer {} sent invalid genesis: {}", peer_id,
                  reason);
        peer_manager_->Misbehaving(peer_id, MisbehaviorPenalty::INVALID_HEADER,
      ...
      // Unknown rejection reason - log and fail (don't punish in case it's our
      // bug)
      LOG_ERROR("HeaderSync: Failed to accept header from peer {} - Hash: {}, "
                "Reason: {}, Debug: {}",
                peer_id, header.GetHash().ToString(), reason,
  ...
  LOG_INFO("HeaderSync: Calling ActivateBestChain for batch of {} headers",
           headers.size());
  validation::ValidationState state;
  bool activate_result = chainstate_manager_.ActivateBestChain(nullptr);
  LOG_INFO("HeaderSync: ActivateBestChain returned {}",
           activate_result ? "true" : "FALSE");
  if (!activate_result) {
    return false;
  }
  ...
  if (chainstate_manager_.IsInitialBlockDownload()) {
    ...
    LOG_INFO("Synchronizing block headers, height: {} (~{:.2f}%)",
             GetBestHeight(), progress * 100.0);
  } else {
    // After IBD: show simple new block notification
    LOG_INFO("New block header: height={}, hash={}...", GetBestHeight(),
             GetBestHash().ToString().substr(0, 16));
  }
...
void HeaderSync::UpdateState() {
  ...
  // Notify callback if state changed (call with mutex held to avoid race on
  // callback itself)
