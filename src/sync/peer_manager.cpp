...
void PeerManager::AddPeer(int peer_id, const std::string& address, NetPermissionFlags permissions)
{
...
bool PeerManager::Misbehaving(int peer_id, int howmuch, const std::string& message)
{
    if (howmuch <= 0) {
        return false;
    }
    ...
    bool threshold_crossed = (score_now >= DISCOURAGEMENT_THRESHOLD && score_before < DISCOURAGEMENT_THRESHOLD);
    ...
    if (threshold_crossed) {
        ...
        if (HasPermission(peer->permissions, NetPermissionFlags::NoBan)) {
            ...
            LOG_WARN("Misbehaving: NoBan peer={} ({}) score: {} -> {} - protected from disconnect{}",
    ...
    if (score_now >= DISCOURAGEMENT_THRESHOLD) {
        LOG_WARN("Misbehaving: peer={} ({}) score: {} -> {}{}{}",
                 peer_id, peer->address, score_before, score_now, warning, message_prefixed);
    } else {
        LOG_INFO("Misbehaving: peer={} ({}) score: {} -> {}{}",
                 peer_id, peer->address, score_before, score_now, message_prefixed);
    }
...
bool PeerManager::IncrementUnconnectingHeaders(int peer_id)
{
    ...
    auto it = peers_.find(peer_id);
    if (it == peers_.end()) {
        LOG_DEBUG("IncrementUnconnectingHeaders called for unknown peer {}", peer_id);
        return false;
    }
    ...
    LOG_DEBUG("Peer {} ({}) unconnecting headers count: {}",
