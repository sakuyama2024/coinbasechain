...
BanMan::BanMan(const std::string& datadir)
    : m_datadir(datadir)
{
    LOG_INFO("BanMan initialized with datadir: {}", datadir.empty() ? "<none>" : datadir);
}
...
bool BanMan::Load()
{
    ...
    try {
        ...
        LOG_INFO("BanMan: Loaded {} bans from {} (skipped {} expired)", loaded, path, expired);
        return true;
...
void BanMan::Ban(const std::string& address, int64_t ban_time_offset)
{
    ...
    if (ban_time_offset > 0) {
        LOG_WARN("BanMan: Banned {} until {} ({}s)", address, ban_until, ban_time_offset);
    } else {
