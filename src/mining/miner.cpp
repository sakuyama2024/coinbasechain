...
bool CPUMiner::Start(int num_threads)
{
    if (mining_.load()) {
    ...
    LOG_INFO("  Previous: {}...", current_template_.hashPrevBlock.ToString().substr(0, 16));
...
void CPUMiner::MiningWorker()
{
    ...
    while (mining_.load()) {
        ...
        bool found_block = consensus::CheckProofOfWork(
            current_template_.header,
            current_template_.nBits,
            params_,
            crypto::POWVerifyMode::MINING,
            &rx_hash
        );
