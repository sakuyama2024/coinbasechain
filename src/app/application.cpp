...
bool Application::initialize() {
    ...
    miner_ = std::make_unique<mining::CPUMiner>(*chain_params_, *chainstate_manager_);
...
void Application::shutdown() {
    ...
    if (network_manager_) {
        ...
        if (!network_manager_->SaveAnchors(anchors_file)) {
            LOG_DEBUG("No anchors to save (this is normal if no peers were connected)");
        }
    }
...
bool Application::init_randomx() {
    ...
    // Initialize with default cache size and light mode
    crypto::InitRandomX(crypto::DEFAULT_RANDOMX_VM_CACHE_SIZE,
                       crypto::DEFAULT_RANDOMX_FAST_MODE);
...
void Application::start_periodic_saves() {
    ...
    save_thread_ = std::make_unique<std::thread>(&Application::periodic_save_loop, this);
}
