...
chain::CBlockIndex* ChainSelector::FindMostWorkChain()
{
    ...
    for (auto it = m_candidates.begin(); it != m_candidates.end(); ++it) {
        ...
        // Skip blocks with failure flags (BLOCK_FAILED_VALID, BLOCK_FAILED_CHILD, etc.)
        // Also skip blocks not validated to at least BLOCK_VALID_TREE level
        //
        // NOTE: IsValid() currently checks BLOCK_FAILED_MASK internally, but we check
        // it explicitly here for defensive programming and clarity. If IsValid() is
        // refactored to only check validity levels, this prevents silent bugs.
...
void ChainSelector::TryAddBlockIndexCandidate(chain::CBlockIndex* pindex,
                                               const chain::BlockManager& block_manager)
{
    ...
    assert(pindex->nHeight >= 0 && "nHeight must be set before adding to candidates");
    assert(pindex->nChainWork > 0 && "nChainWork must be set before adding to candidates");
...
void ChainSelector::PruneBlockIndexCandidates(const chain::BlockManager& block_manager)
{
    ...
    chain::CBlockIndex* pindexTip = const_cast<chain::CBlockIndex*>(pindexTipConst);
...
void ChainSelector::ClearCandidates()
{
    m_candidates.clear();
}
