                                ...
                                ValidationState& state)
{
    ...
    if (header.nBits != expected_bits) {
        return state.Invalid("bad-diffbits",
            "incorrect difficulty: expected " + std::to_string(expected_bits) +
            ", got " + std::to_string(header.nBits));
    }
    ...
    if (header.nVersion < 1) {
        return state.Invalid("bad-version",
            "block version too old: " + std::to_string(header.nVersion));
    }
...
int64_t GetAdjustedTime()
{
    ...
    // - Mitigates eclipse attacks where attacker controls victim's time perception
    //
...
arith_uint256 CalculateHeadersWork(const std::vector<CBlockHeader>& headers)
{
    ...
    for (const auto& header : headers) {
        ...
        // format allows encoding this (mantissa can be 0), but it's consensus-invalid.
        //
        // While the formula (~bnTarget / (bnTarget + 1)) is mathematically safe when
        // bnTarget=0 (divides by 1), we still reject it as an invalid difficulty target.
        // Such blocks should never appear in a valid chain and are filtered here.
