    ...
    try {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (arg == "--help") {
                ...
                config.network_config.network_magic = coinbasechain::protocol::magic::REGTEST;
                config.network_config.listen_port = coinbasechain::protocol::ports::REGTEST;
            }
            else if (arg == "--testnet") {
                config.chain_type = coinbasechain::chain::ChainType::TESTNET;
                config.network_config.network_magic = coinbasechain::protocol::magic::TESTNET;
                config.network_config.listen_port = coinbasechain::protocol::ports::TESTNET;
            }
            else if (arg == "--verbose") {
                config.verbose = true;
                log_level = "debug";
        ...
        // Use std::cerr here because logger may not be safe during exception handling
