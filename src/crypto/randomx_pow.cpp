...
// Global caches
static std::unique_ptr<LRUCache<uint32_t, std::shared_ptr<RandomXCacheWrapper>>> g_cache_rx_cache;
static std::unique_ptr<LRUCache<uint32_t, std::shared_ptr<RandomXVMWrapper>>> g_cache_rx_vm_light;
...
void InitRandomX(int vmCacheSize, bool fastMode)
{
    ...
    g_cache_rx_cache = std::make_unique<LRUCache<uint32_t, std::shared_ptr<RandomXCacheWrapper>>>(vmCacheSize);
    g_cache_rx_vm_light = std::make_unique<LRUCache<uint32_t, std::shared_ptr<RandomXVMWrapper>>>(vmCacheSize);

    g_randomx_initialized = true;

    LOG_CRYPTO_INFO("RandomX initialized (cache size: {}, light mode only)", vmCacheSize);
}
...
randomx_vm* CreateVMForEpoch(uint32_t nEpoch)
{
    ...
    std::shared_ptr<RandomXCacheWrapper> myCache;
    {
        ...
        if (g_cache_rx_cache->contains(nEpoch)) {
            ...
            LOG_CRYPTO_INFO("Created RandomX cache for epoch {} (for parallel verification)", nEpoch);
        }
    }
    ...
    if (!vm) {
        throw std::runtime_error("Failed to create RandomX VM for parallel verification");
    }
